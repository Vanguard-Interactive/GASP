...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GASPTraversableObstacleComponent)

void UGASPTraversableObstacleComponent::Initialize(const TArray<USplineComponent*>& NewLedges,
...
USplineComponent* UGASPTraversableObstacleComponent::FindLedgeClosestToActor(const FVector& ActorLocation) const
{
	if (Ledges.IsEmpty())
	{
		return nullptr;
...
void UGASPTraversableObstacleComponent::GetLedgeTransforms_Implementation(const FVector& HitLocation,
